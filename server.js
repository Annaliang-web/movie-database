//open server in browser enter this in Terminal:  nodemon server.js 8082
//enter localurl http://localhost:8082/

// initialize Express in project
const express = require('express');
const app = express();
const bodyParser = require('body-parser'); //handle app.post body request
// npm install request - g
const request = require('request');

app.set('view engine', 'ejs');
app.use(bodyParser.urlencoded({enxtended: false}));
app.use(express.static('public'));  // to connect CSS, HTML or any files that open to public!!!

app.get('/', (req, res) => {
    // let allData = {
    //   movieList: getmovies()
    // }
  res.render('pages/index.ejs', { movieList: getmovies()}); //same as passing allData object
})

app.get('/movie/:movieId', (req, res) => {
    /* render pages/movie with data of specfic movie */
  let movieData = { 
    movieName: req.params.movieId, 
    movieImg:  req.params.movieId.poster,
    movieList: getmovies()
  }
  res.render('pages/movie.ejs', movieData);
})

// app.get('/photos', (req,res)=>{ //browser asking me to send request app.get
//   let api = 'https://api.themoviedb.org/3/discover/movie?api_key=1555dd396b0c44c8d64150a64dcb3435&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1'
//   let allPhoto = {photoArr: []};
//   request(api, function (error, response,body) { //I am sending request to ask 3rd party server
//     if(!error){
//       let bodyObj = JSON.parse(body);
//       for(let i=0; i < bodyObj.length;i++){
//         photoArr = bodyObj.results[i].poster_path
//       }
//       console.log(bodyObj.results[0].poster_path)
//     }
  
//   })
//   res.render('pages/photos.ejs',{photos:});
// })

app.post('/search', function (req,res){
  let searchMovies = req.body.searchMovies.toLowerCase();
  let searchResults = [];
  let movieList = getmovies();
  for(let i = 0; i< movieList.length; i++){
    if (movieList[i].title.toLowerCase().indexOf(searchMovies) >= 0){
        searchResults.push(movieList[i]);
      }
    else if (movieList[i].year.indexOf(searchMovies) >= 0) {
      searchResults.push(movieList[i]);
    }
    else if (movieList[i].director.toLowerCase().indexOf(searchMovies) >= 0){
        searchResults.push(movieList[i]);
      }  
  }
  res.render('pages/searchResults', {movieList: searchResults});
});



// start Express on port 8082
app.listen(8082, () => {
...
// console.log(Object.keys(getmovies()[0]));
